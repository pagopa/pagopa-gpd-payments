...
public class PaymentsService {
  ...
  public PaymentsService(
      String storageConnectionString, String receiptsTable, GpdClient gpdClient) {
    ...
    try {
      ...
      String partitionKeyFilter =
          TableQuery.generateFilterCondition(
              PARTITION_KEY_FIELD, TableQuery.QueryComparisons.EQUAL, organizationFiscalCode);
      String rowKeyFilter =
          TableQuery.generateFilterCondition(ROW_KEY_FIELD, TableQuery.QueryComparisons.EQUAL, iuv);

      String filter = TableQuery.combineFilters(partitionKeyFilter, Operators.AND, rowKeyFilter);

      Iterable<ReceiptEntity> result =
          table.execute(TableQuery.from(ReceiptEntity.class).where(filter));
      ...
      log.error(
          "[getReceiptByOrganizationFCAndIUV] Payments Generic Error "
  ...
  public PaymentsResult<ReceiptEntity> getOrganizationReceipts(
    ...
    try {
      ...
      if (null != debtor) {
        String debtorFilter =
            TableQuery.generateFilterCondition(
                DEBTOR_FIELD, TableQuery.QueryComparisons.EQUAL, debtor);
      ...
      log.error(
          "[getReceiptByOrganizationFCAndIUV] Payments Generic Error "
              + String.format(LOG_BASE_PARAMS_DETAIL, organizationFiscalCode, debtor, service),
          e);
  ...
  public void checkGPDDebtPosStatus(CloudTable table, ReceiptEntity receipt) {
    ...
    if (!receipt.getStatus().trim().equalsIgnoreCase(Status.PAID.name())) {
      PaymentsModelResponse paymentOption =
          gpdClient.getPaymentOption(receipt.getPartitionKey(), receipt.getRowKey());
      ...
      try {
        ...
        log.error(
            "[checkGPDDebtPosStatus] Non-blocking error: "
                + "Exception during the update status in table "
                + receiptsTable
                + " for ReceiptEntity [pk:"
                + receipt.getPartitionKey()
                + ", rk:"
                + receipt.getRowKey()
                + "]",
  ...
  public PaymentsResult<ReceiptEntity> getGPDCheckedReceiptsList(
      CloudTable table, PaymentsResult<ReceiptEntity> result) {
  ...
  private PaymentsResult<ReceiptEntity> getSegmentedReceipts(
      Integer limit,
      Integer pageNum,
    ...
    // first page results
    ResultSegment<ReceiptEntity> segmentedRes =
        table.executeSegmented(tq.select(columns).where(filter).take(limit), null);
    ...
    if (pageNum > 0) {
      for (int i = 1; i <= pageNum; i++) {
        if (segmentedRes.getHasMoreResults()) {
          segmentedRes =
              table.executeSegmented(
                  TableQuery.from(ReceiptEntity.class).where(filter).take(limit),
                  segmentedRes.getContinuationToken());
          result.setLength(segmentedRes.getLength());
          result.setPageSize(segmentedRes.getPageSize());
  ...
  private PaymentsResult<ReceiptEntity> getNotSegmentedReceipts(CloudTable table, String filter) {
    ...
    tq.setColumns(columns);
    PaymentsResult<ReceiptEntity> result = new PaymentsResult<>();
    List<ReceiptEntity> listOfEntity =
        StreamSupport.stream(table.execute(tq.select(columns).where(filter)).spliterator(), false)
  ...
  private static String getStartsWithFilter(String columnName, String startsWith) {
    ...
    return TableQuery.combineFilters(
        TableQuery.generateFilterCondition(
            columnName, QueryComparisons.GREATER_THAN_OR_EQUAL, startsWith),
        Operators.AND,
