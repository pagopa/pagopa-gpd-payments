...
class PaymentValidatorTest {
  ...
  @Test
  void isAuthorize_OK() {
    when(apiConfigClient.getOrganization(anyString(), anyString()))
        .thenReturn(new StationCreditorInstitution());
  ...
  void isAuthorize_apiConfigCallFail() {
    ...
    Request request =
        Request.create(
            ...
            new RequestTemplate());
    when(apiConfigClient.getOrganization(anyString(), anyString()))
        .thenThrow(new FeignException.NotFound("error", request, null, null));

    try {
      paymentValidator.isAuthorize("aaa", "bbb", "ccc");
    } catch (PartnerValidationException e) {
      assertThat(e.getError().getFaultCode())
          .isEqualTo(PaaErrorEnum.PAA_ID_DOMINIO_ERRATO.getFaultCode());
    }
  }
}
